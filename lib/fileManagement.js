...
function FileObj(config) {
	...
	function enumerateLibraries() {
		...
		for (var libs in source) {
			if (source.hasOwnProperty(libs)) {
				...
				var promise = enumeratePackages(currLib[libs], libName, libFolder);
	...
	function enumeratePackages(pkgs, libName, libFolder) {
		...
		for (var pkg in pkgs) {
			if (pkgs.hasOwnProperty(pkg)) {
				...
				var promise = arrayOfGlob(pkgs, bowerFileFolder, libName, 0, localCache).then(
					function(pkgList, lc) {
						var underPromise = enumerateFile(pkgList, lc.fileName, libFolder, lc.fileFolder, lc.pkg === "!" ? "ignore" : "move");
						underPromise.then(
							function(data) {
					...
					if (err) {
						...
						for (var file in files) {
							if (files.hasOwnProperty(file)) {
								promises.push(deleteFile(file, position, top));
							}
						}
					}
				});
	 ...
	 * @param top {boolean}
	 */
	function deleteFile(file, position, top) {
